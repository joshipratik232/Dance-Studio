...
class DotProgressIndicator : FrameLayout {
    ...
    private lateinit var dotProgressIndicator: LinearLayout
    ...
    private fun init() {
        clipChildren = false
        clipToPadding = false
        dotProgressIndicator = LinearLayout(context)
        val progressBarLayoutParams = LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT)
        progressBarLayoutParams.gravity = Gravity.CENTER
        dotProgressIndicator.layoutParams = progressBarLayoutParams
        dotProgressIndicator.clipChildren = false
        dotProgressIndicator.clipToPadding = false
        addView(dotProgressIndicator)
        animators.clear()
        for (i in 0 until numberOfDots) {
            ...
            dotProgressIndicator.addView(dot)
    ...
    class Builder {
        ...
        private var primaryAnimator: ValueAnimator? = null
        private var dotBackground = R.drawable.ic_dot
        fun build(context: Context): DotProgressIndicator {
            val dotProgressBar = DotProgressIndicator(context)
